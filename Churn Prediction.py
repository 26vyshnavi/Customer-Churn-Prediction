...
scaler = StandardScaler()  # Initialize StandardScaler for scaling numerical features
...
def predict(data):
    ...
    rf_model = RandomForestClassifier()  # Initialize RandomForestClassifier
    rf_model.fit(X_train, y_train)  # Train the model

    # Load the XGBoost model
    xgb_model = xgb.XGBClassifier()  # Initialize XGBoost model
    ...
    predictions_rf = rf_model.predict(data)
    predictions_xgb = xgb_model.predict(data)

    return predictions_rf, predictions_xgb

# Streamlit UI
st.title('Machine Learning Model Deployment')
st.write('This web app deploys machine learning models.')
...
# Example input data
example_data = {
    'CreditScore': st.sidebar.number_input('Credit Score', value=600),
    'Geography': st.sidebar.selectbox('Geography', ['France', 'Spain', 'Germany']),
    'Gender': st.sidebar.selectbox('Gender', ['Female', 'Male']),
    'Age': st.sidebar.number_input('Age', value=30),
    'Tenure': st.sidebar.number_input('Tenure', value=3),
    'Balance': st.sidebar.number_input('Balance', value=0),
    'NumOfProducts': st.sidebar.number_input('Number of Products', value=1),
    'HasCrCard': st.sidebar.checkbox('Has Credit Card'),
    'IsActiveMember': st.sidebar.checkbox('Is Active Member'),
    'EstimatedSalary': st.sidebar.number_input('Estimated Salary', value=50000)
}
...
if st.sidebar.button('Predict'):
    # Make predictions
    predictions_rf, predictions_xgb = predict(input_df)
    
    # Display predictions
    st.subheader('Predictions')
    st.write('Random Forest Model:', predictions_rf)
    st.write('XGBoost Model:', predictions_xgb)
